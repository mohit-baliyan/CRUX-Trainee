...
public class BinaryTree {
	...
	private Node construct(int[] posto, int[] ino, int psi, int pei, int insi, int inei) {
		if (psi > pei) {
			return null;
		}
		...
		int nofelonls = idx - insi;

		// set its left
		node.left = construct(posto, ino, psi, psi + nofelonls - 1, insi, idx - 1);

		// set its right
		node.right = construct(posto, ino, psi + nofelonls, pei - 1, idx + 1, inei);
	...
	public void levelolwzz() {
		LinkedList<Node> cl = new LinkedList<>(); // like a queue
		LinkedList<Node> nl = new LinkedList<>(); // like a stack
		boolean l2r = true;
		...
		while (cl.size() != 0) {
			...
			if (l2r == true) {
				if (temp.left != null) {
					nl.addFirst(temp.left);
				}

				if (temp.right != null) {
					nl.addFirst(temp.right);
				}
			} else {
				if (temp.right != null) {
					nl.addFirst(temp.right);
	...
	public ArrayList<LinkedList<Integer>> levelolwLL() {
		...
		LinkedList<Integer> lwlist = new LinkedList<>();
		while (cl.size() != 0) {
			// 1.
			Node temp = cl.removeFirst();

			// 2.
			lwlist.addLast(temp.data);
	...
	private void roottoleafpaths(Node node, String psf, int ssf) {
		if (node == null) {
			return;
		}
		...
		roottoleafpaths(node.left, psf + node.data + " -> ", ssf + node.data);
		roottoleafpaths(node.right, psf + node.data + " -> ", ssf + node.data);
	}

	public ArrayList<Integer> rootToDataPath(int data){
		return rootToDataPath(root, data);
	}
	
	private ArrayList<Integer> rootToDataPath(Node node, int data){
		...
		ArrayList<Integer> llist = rootToDataPath(node.left, data);
		if(llist.size() > 0){
			mylist.add(node.data);
			mylist.addAll(llist);
			return mylist;
		...
		ArrayList<Integer> rlist = rootToDataPath(node.right, data);
		if(rlist.size() > 0){
			mylist.add(node.data);
			mylist.addAll(rlist);
			return mylist;
	...
	public void lca(int d1, int d2){
		ArrayList<Integer> rtod1path = rootToDataPath(d1);
		ArrayList<Integer> rtod2path = rootToDataPath(d2);
		int i = 0;
		for(i = 0; i < rtod1path.size() && i < rtod2path.size(); i++){
			if(rtod1path.get(i) != rtod2path.get(i)){
				break;
			}
		}
		
		System.out.println(rtod1path.get(i - 1));
	}
}
