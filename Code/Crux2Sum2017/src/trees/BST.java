...
public class BST {
	...
	public void printTargetSum(int tar) {
		LinkedList<OrderPair> sfl = new LinkedList<>();
		LinkedList<OrderPair> sfr = new LinkedList<>();

		sfl.addFirst(new OrderPair(this.root));
		sfr.addFirst(new OrderPair(this.root));
		
		OrderPair leftSelection = null;
		OrderPair rightSelection = null;
		
		while (true) {
			while (sfl.size() != 0) {
				OrderPair temp = sfl.getFirst();

				if (temp.leftDone == false) {
					if (temp.node.left != null) {
						OrderPair leftPair = new OrderPair(temp.node.left);
						sfl.addFirst(leftPair);
					}
					temp.leftDone = true;
				} else if (temp.selfDone == false) {
					leftSelection = temp;
			...
			while (sfr.size() != 0) {
				OrderPair temp = sfr.getFirst();

				if (temp.rightDone == false) {
					if (temp.node.right != null) {
						OrderPair rightPair = new OrderPair(temp.node.right);
						sfr.addFirst(rightPair);
					}
					temp.rightDone = true;
				} else if (temp.selfDone == false) {
					rightSelection = temp;
			...
			if(leftSelection.node.data >= rightSelection.node.data){
				break;
			}
			
			if(leftSelection.node.data + rightSelection.node.data == tar){
				System.out.println(leftSelection.node.data + ", " + rightSelection.node.data);
				leftSelection.selfDone = true;
				rightSelection.selfDone = true;
			} else if(leftSelection.node.data + rightSelection.node.data < tar){
				leftSelection.selfDone = true;
			} else {
				rightSelection.selfDone = true;
			}
