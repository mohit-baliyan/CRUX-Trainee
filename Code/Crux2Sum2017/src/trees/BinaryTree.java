...
public class BinaryTree {
	...
	public void kaway(int k, int data) {
		System.out.println(find(root, k, data));
	}

	private int find(Node node, int k, int data) {
		if (node == null) {
			return -1;
		}
		...
		int dfl = this.find(node.left, k, data);
		if (dfl != -1) {
			if (dfl + 1 == k) {
		...
		int dfr = this.find(node.right, k, data);
		if (dfr != -1) {
			if (dfr + 1 == k) {
				System.out.println(node.data);
			} else {
	...
	public void roottoleafpaths(int tar){
		roottoleafpaths(root, "", tar);
	}
	
	private void roottoleafpaths(Node node, String psf, int tar){
		if(node == null){
			return;
		}
		
		if(node.left == null && node.right == null && tar == node.data){
			System.out.println(psf + node.data);
			return;
		}
		
		roottoleafpaths(node.left, psf + node.data + "=> ", tar - node.data);
		roottoleafpaths(node.right, psf + node.data + "=>", tar - node.data);
	}
