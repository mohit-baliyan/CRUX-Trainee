...
public class GenericTree {
	...
	public boolean iso(GenericTree other){
		return this.iso(this.root, other.root);
	}
	
	private boolean iso(Node tnode, Node onode){
		...
		for(int i = 0; i < tnode.children.size(); i++){
			...
			if(this.iso(tchild, ochild) == false){
				return false;
			}
		}
	...
	// is the tree and its mirror image having the same structure
